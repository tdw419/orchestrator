...
function classifyError(res) {
    try {
        ...
        if (typeof status === 'number' && status === 408)
            return 'timeout';
        if (stderr.includes('timeout') || errStr.includes('timeout'))
            return 'timeout';
        if (stderr.includes('permission') || errStr.includes('permission') || stderr.includes('access denied'))
            return 'permission';
        if (stderr.includes('network') || errStr.includes('connection') || errStr.includes('unreachable'))
            return 'network';
        if (stderr.includes('memory') || errStr.includes('out of memory') || stdout.includes('heap'))
            return 'resource';
        if (stderr.includes('syntax') || errStr.includes('parse') || errStr.includes('invalid'))
            return 'syntax';
        if (stderr.includes('not found') || errStr.includes('missing') || errStr.includes('does not exist'))
...
function getRetryStrategy(errorType) {
    ...
    if (t.includes('syntax'))
        return 'fix_syntax';
    if (t.includes('missing_dependency'))
...
function checkProgress(prevRes, currRes) {
    try {
        if (!prevRes || !currRes)
            return true; // First attempt or missing data
        const prev = prevRes.data || {};
        const curr = currRes.data || {};
        // Status improvement
        if (prevRes.status > currRes.status)
            return true;
        // Exit code improvement
        if (prev.exitCode > 0 && curr.exitCode === 0)
            return true;
        if (prev.exitCode > curr.exitCode && curr.exitCode >= 0)
            return true;
        // Error presence changes
        if (prev.error && !curr.error)
            return true;
        if (prev.ok === false && curr.ok === true)
            return true;
        ...
        if (prevOutput !== currOutput && prevOutput.length > 0)
            return true;
        return false;
    }
    catch (_) {
...
async function callLLM(contextStr) {
    const system = [
...
function resolveTemplateGoal(template, inputs = {}) {
...
async function callDesktop(action, params) {
    ...
    if (action === 'run_powershell') {
        ...
        if (!script)
            return { status: 400, data: { error: 'missing_script' } };
    ...
    if (action === 'verify_result') {
        ...
        try {
            if (method === 'file_exists') {
                ...
                if (!p)
                    return { status: 400, data: { error: 'missing_path' } };
          ...
          `;
                ...
                if (!script)
                    return { status: 400, data: { error: 'invalid_test_type_or_missing_command' } };
                const ps = spawn('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', script], { windowsHide: true });
                let stdout = '', stderr = '';
                ps.stdout.on('data', (d) => (stdout += d.toString()));
                ps.stderr.on('data', (d) => (stderr += d.toString()));
            ...
            return { status: 500, data: { error: `verify_result failed: ${e.message}` } };
        }
    }
    ...
    try {
        ...
        return { status: 503, data: { error: 'desktop_driver_unreachable', message: String(e?.message || e) } };
    }
}
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && url.pathname === '/tasks') {
            try {
                ...
                if (!goal)
                    return sendJson(res, 400, { error: 'missing_goal' });
                const id = randomUUID();
                const task = {
                    id,
                    goal,
        ...
        if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/viewer')) {
            try {
                ...
                return sendJson(res, 500, { error: 'viewer_error', message: String(e?.message || e) });
            }
        }
...
async function runTaskLoop(task, depth = 0) {
    task.status = 'running';
    task.depth = depth;
    ...
    for (let i = 0; i < config.maxSteps; i++) {
        try {
            ...
            const step = { i: i + 1, planned: action, attempts: [], context: { learned_issues: [], attempted_fixes: [] } };
            ...
            while (attempt <= config.maxRetriesPerStep) {
                ...
                const attemptInfo = { n: attempt + 1, action, result: res };
