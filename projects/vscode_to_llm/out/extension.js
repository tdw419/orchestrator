...
function activate(context) {
    ...
    const chatProvider = new chatView_1.ChatViewProvider(context);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(chatView_1.ChatViewProvider.viewType, chatProvider));
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('lmstudio.createTask', createTask), vscode.commands.registerCommand('lmstudio.viewTasks', viewTasks), vscode.commands.registerCommand('lmstudio.startOrchestrator', startOrchestrator), vscode.commands.registerCommand('lmstudio.stopOrchestrator', stopOrchestrator), vscode.commands.registerCommand('lmstudio.openChat', async () => {
        try {
...
function updateOrchestratorClient() {
    ...
    const baseUrl = config.get('orchestrator.baseUrl', 'http://localhost:4100');
    orchestratorClient = new orchestratorClient_1.OrchestratorClient(baseUrl);
}
async function createTask() {
    try {
        ...
        if (result && result.id) {
            ...
            const viewTask = await vscode.window.showInformationMessage('Task created successfully!', 'View Tasks', 'Open Orchestrator');
...
async function askAboutSelection(chatProvider) {
...
async function startOrchestrator() {
    ...
    try {
        const config = vscode.workspace.getConfiguration('lmstudio');
        const entryPath = config.get('orchestrator.entryPath', '');
        const orchestratorPort = config.get('orchestrator.port', 4100);
        const apiUrl = config.get('apiUrl', 'http://localhost:1234/v1');
        const modelName = config.get('modelName', 'lmstudio-local');
        const maxSteps = config.get('maxSteps', 8);
        if (!entryPath) {
        ...
        orchestratorProcess = (0, child_process_1.spawn)('node', [entryPath], {
            env,
            cwd: workingDir,
            detached: false,
...
async function developWithAI() {
    try {
        ...
        const baseUrl = cfg.get('orchestrator.baseUrl', 'http://localhost:4100');
        const adminToken = cfg.get('orchestrator.adminToken', '');
        const apiUrl = cfg.get('apiUrl', 'http://localhost:1234/v1');
        const modelName = cfg.get('modelName', 'lmstudio-local');
        ...
        const payload = { config: { prompt: prompt.trim(), project_dir: target.path } };
        if (apiUrl) {
...
async function buildPromptViaTemplates(defaultSuggestion) {
    const entries = [];
    // Templates section
...
async function pickTargetDirectory() {
    ...
    const pathAbs = (rel) => (require('path').isAbsolute(rel) ? rel : require('path').join(root, rel));
    // Recent targets
...
function getRecentPrompts() {
    if (!extContext)
        return [];
    try {
        const list = extContext.globalState.get(RECENT_PROMPTS_KEY, []) || [];
...
function saveRecentPrompts(list) {
    if (!extContext)
        return;
    extContext.globalState.update(RECENT_PROMPTS_KEY, list);
}
function addRecentPrompt(text) {
    const t = (text || '').trim();
    if (!t)
        return;
    const now = Date.now();
...
function getRecentTargets() {
    if (!extContext)
        return [];
    try {
        const list = extContext.globalState.get(RECENT_TARGETS_KEY, []) || [];
        return Array.isArray(list) ? list : [];
    }
    catch {
        return [];
...
function addRecentTarget(path, label) {
    const p = (path || '').trim();
    if (!p)
        return;
...
function defaultPromptForTarget(target) {
    const name = (target.label || '').toLowerCase();
    if (name.includes('vscode') || name.includes('extension'))
        return 'Add feature to VS Code extension';
    if (name.includes('script'))
        return 'Create PowerShell automation script';
    if (name.includes('orchestrator'))
        return 'Enhance orchestrator core functionality';
    return undefined;
}
function deactivate() {
